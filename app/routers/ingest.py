...
async def init_db(session: AsyncSession = Depends(get_db_session)) -> dict[str, str]:
    # Create all tables
...
async def ingest_from_json(
    ...
    if not base_path.exists():
        raise HTTPException(status_code=400, detail=f"Base dir not found: {base_dir}")

    import json

    groups_path = base_path / "product_groups.json"
    variants_path = base_path / "product_variants.json"
    required_components_path = base_path / "required_components.json"

    if not groups_path.exists():
        raise HTTPException(status_code=400, detail="product_groups.json missing")
    if not variants_path.exists():
        raise HTTPException(status_code=400, detail="product_variants.json missing")

    groups_json: dict[str, Any] = json.loads(groups_path.read_text())
    variants_json: dict[str, Any] = json.loads(variants_path.read_text())
    ...
    # Variants (expects a single group's variants for MVP)
    inserted_variants = 0
    ...
    if required_components_json:
        ...
        for c in required_components_json.get("components", []):
            ...
            existing_id = desc_to_component_id.get(description)
            if not existing_id:
